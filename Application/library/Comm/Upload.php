...
class Upload {
    ...
    private $allowtype = array('xls', 'xlsx'); //充许上传文件的类型
    private $maxsize = 20971520; //允上传文件的最大长度 20M
    ...
    private function getError() {
        $str = "上传 {$this->originName} 出错咯: ";
        switch ($this->errorNum) {
            case 4: $str .= '没有文件被上传';
                break;
            case 3: $str .= '文件只被部分上传';
                break;
            case 2: $str .= '文件超过了HTML表单中MAX_FILE_SIZE选项指定的值';
                break;
            case 1: $str .= '文件超过了php上传许可值';
                break;
            case -1: $str .= '未许可的类型';
                break;
            case -2: $str .= '文件过大,不能超过{$this->$maxsize}个字节';
                break;
            case -3: $str .= '上传失败';
                break;
            case -4: $str .= '建立上传目录失败,请重新指定上传目录';
                break;
    ...
    private function proRandName() {
        $fileName = date("Ymd") . rand(100, 999);
