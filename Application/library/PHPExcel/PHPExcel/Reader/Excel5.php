...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function load($pFilename)
    {
        ...
        if (!$this->readDataOnly && $this->drawingGroupData) {
            ...
            $escherWorkbook = $reader->table($this->drawingGroupData);

            // debug Escher stream
            //$debug = new Debug_Escher(new PHPExcel_Shared_Escher());
            //$debug->table($this->drawingGroupData);
        }
        ...
        foreach ($this->sheets as $sheet) {
            ...
            if (!$this->readDataOnly && $this->drawingData) {
                ...
                $escherWorksheet = $reader->table($this->drawingData);

                // debug Escher stream
                //$debug = new Debug_Escher(new PHPExcel_Shared_Escher());
                //$debug->table($this->drawingData);
