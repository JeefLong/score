 ...
 * Index controller
 */
Final class GoldController extends BaseController {
...
// 获得View渲染对象
     ...
     * 订单集合
     */
    public function ListGoldAction() {
        $mod = DbModel::Init();
        $perpage = 10;
        ...
        $cont = $mod->table('order_gold')->get_count($s_arr);
        $ret = $mod->table('order_gold')->get_list('*', $s_arr, ($p - 1) * $perpage, $perpage);
        ...
        $this->view->display('Admin/Gold/list_order.html');
        return TRUE;
    }

    // 数量统计
    public function SumGoldAction() {
        $perpage = 10;
        $p = isset($this->req['p']) ? (intval($this->req['p']) < 1) ? 1 : intval($this->req['p']) : 1;
        $status = isset($this->req['status']) ? trim($this->req['status']) : '';
        ...
        $sql = 'SELECT `id`, `agent_id`, SUM(IF(game_gold < 0,game_gold,0)) AS `put`, sum(IF(game_gold > 0,game_gold,0)) AS `get` FROM `order_gold` ';
        $sql .= $wh;
        ...
        $mod = DbModel::Init();
        $cd = $mod->table('order_gold')->query_data($count);
        $cout = $cd[0]['COUNT'];
        $ret = $mod->table('order_gold')->query_data($sql);
        $ret = $this->GetAgtname($ret);
        ...
        $this->view->display('Admin/Gold/list_sum.html');
    }

    // 删除
    public function AjaxDelGoldAction() {
        ...
        foreach ($uid_arr as $k => $v) {
            $ret = $mod->table('order_gold')->get_one('*', array('id' => $v));
            if (isset($ret['id'])) {
                unset($ret['id']);
            }
            $rst = $mod->table('order_gold_t')->add_data($ret);
            $rs = $mod->table('order_gold')->del_data(array('id' => $v));
            if ($rst && $rs) {
                $flag = 1;
    ...
    private function GetAgtname($ret) {
        ...
        if (isset($ret['id']) && $ret['id'] > 0) {
            if (is_null($ret['agent_id'])) {
                ...
                $ret['agt_name'] = is_null($res['agent_name']) ? '(已删除)ID:' . $res['agent_id'] : $res['agent_name'];
            }
        ...
        foreach ($ret as $k => $v) {
            if (is_null($v['agent_id'])) {
                ...
                $ret[$k]['agt_name'] = is_null($res['agent_name']) ? '(已删除)ID:' . $v['agent_id'] : $res['agent_name'];
    ...
    private function GetOpname($ret) {
        ...
        if (isset($ret['id']) && $ret['id'] > 0) {
            if (is_null($ret['op_agent_id'])) {
                ...
                $ret['op_name'] = is_null($res['agent_name']) ? '(已删除)ID:' . $res['op_agent_id'] : $res['agent_name'];
            }
            //$ret['agt_gold'] = $res['card'];
            return $ret;
        }
        foreach ($ret as $k => $v) {
            if (is_null($v['agent_id'])) {
                ...
                $res = $mod->table('angent_info')->get_one('agent_name,card', array('id' => $v['agent_id']));
                $ret[$k]['op_name'] = is_null($res['agent_name']) ? '(已删除)ID:' . $v['agent_id'] : $res['agent_name'];
                //$ret[$k]['agt_gold'] = $res['card'];
            }
        }
        unset($k, $v);
