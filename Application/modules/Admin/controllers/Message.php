...
Final class MessageController extends BaseController {
    ...
    public function AjaxGetMessageAction() {
        $wh_arr['recive_id'] = -1;
        $wh_arr['prive[>=]'] = self::$session['roleid'];
        $wh_arr['is_read'] = 0;
        $mod = DbModel::Init();
        $count = $mod->table('angent_message')->get_count($wh_arr, 0, 7, array('add_time' => 'DESC'));
        $ret = $mod->table('angent_message')->get_list('*', $wh_arr, 0, 7, array('add_time' => 'DESC'));
    ...
    public function ListMessageAction() {
        ...
        $wh_arr = ' `id` = `topic_id`  AND `prive` >= ' . self::$session['roleid'];
        ...
        $this->view->display('Admin/Message/List_Message.html');
    }
    ...
    public function ShowMessageAction() {
        ...
        /** 更新为已读 * */
        $wh = '`topic_id` = ' . $tid . ' AND `prive`=' . self::$session['roleid'] . ' AND `recive_id` = -1';
        $mod->table('angent_message')->up_data(array('is_read' => 1), $wh);
        /** 更新为已读 * */
        $wh_arr = '`topic_id` =' . $tid . ' AND `prive` >= ' . self::$session['roleid'];
        $wh_arr .= ' AND (`recive_id` = -1 OR `send_id` = -1)';
        ...
        $this->view->display('Admin/Message/Show_Message.html');
        return TRUE;
    }
    ...
    public function AjaxSubMessageAction() {
        ...
        if (!empty($ret['send_id']) && !empty($ret['recive_id'])) {
            ...
            $send_id = '-1';
            if ($ret['send_id'] > 0) {
                $recive_id = $ret['send_id'];
            }
            if ($ret['recive_id'] > 0) {
                $recive_id = $ret['recive_id'];
            }
            //***********************//
    ...
    public function AjaxAddMessageAction() {
        $aid = empty($this->req['aid']) ? NULL : intval($this->req['aid']);
        $content = empty($this->req['content']) ? NULL : trim($this->req['content']);
        ...
        $aret = $this->get_agt($aid);
        if (empty($aret['id'])) {
            self::OutMsg(array('ecode' => 400, 'emsg' => '代理ID错误'));
        }
